...
- 运行器：由 `.cargo/config.toml` 指定 `runner = "bash tools/isolarail-runner"`，复用本仓 Local USB CLI/devd 烧录路径。
...
- 推荐使用 `espup` 安装 ESP Rust 工具链：
  ...
  3) 安装 `espflash`：`cargo install espflash`（由 `isolarail-devd` 后端调用）；
...
- `cargo run --release` 会通过 `tools/isolarail-runner` 复用同一 Local USB 身份校验和烧录路径。
...
- 现有 GitHub Actions（如 `.github/workflows/check.yml`、`release.yml`、`dev-release.yml`）仍指向 `thumbv7em-none-eabihf` 与二进制名 `isolarail`（STM32 相关）。
...
- 运行时验证以 `just flash-monitor` 或 `cargo run --release`（通过 `isolarail` / `isolarail-devd`）串口日志为主；请在 PR/变更说明中给出期望输出或观测结果。

## 常见问题

- `cargo +esp` 无法运行：确认已通过 `espup install` 安装 `esp` 工具链并 `source ~/export-esp.sh`。
- `isolarail` 未找到设备：先运行 `just ports`，再用 `PORT=/dev/cu.xxx just identify` 明确端口；全新硬件或下载模式需经 `PORT=/dev/cu.xxx just flash-first-time` 的 typed confirmation。
