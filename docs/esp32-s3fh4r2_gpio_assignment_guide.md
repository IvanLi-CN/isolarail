...
- **GPIO17**: EN1 (通道1 SC8815 启停控制，高电平启用)
- **GPIO18**: EN2 (通道2 SC8815 启停控制，高电平启用)
- **GPIO39**: EN3 (通道3 SC8815 启停控制，高电平启用，物理引脚44/MTCK)
- **GPIO40**: EN4 (通道4 SC8815 启停控制，高电平启用，物理引脚45/MTDO)
...
- 其中 `GPIO33`/`GPIO34` 当前已分配给 `UCM_DIN`/`UCM_DCE`，`GPIO35`/`GPIO36`/`GPIO37` 仍可分配。
...
| **电源管理复位** | 1个 | GPIO38 |
| **系统控制** | 2个 | EN, GPIO0 |
| **额外可用IO** | 3个 | GPIO35,36,37 (ESP32-S3FH4R2特有) |
| **预留IO** | 2个 | GPIO47,48 |
...
| **GPIO17** | 23 | EN1 | SC8815 通道1 启停控制 | 高电平启用 |
| **GPIO18** | 24 | EN2 | SC8815 通道2 启停控制 | 高电平启用 |
...
| **GPIO39** | 44 | EN3 | SC8815 通道3 启停控制 | 高电平启用（MTCK） |
| **GPIO40** | 45 | EN4 | SC8815 通道4 启停控制 | 高电平启用（MTDO） |
...
3. **优先推荐**: GPIO35-37、GPIO47-48 (当前可优先分配的通用IO引脚)
...
1. **更多可用GPIO**: 由于使用Quad SPI模式，GPIO33-37可用；当前 `GPIO33/34` 已用于 UCM 控制，仍有 `GPIO35-37` 与 `GPIO47/48` 可调配
...
2. **端口管理**: EN1-4 (GPIO17/18/39/40) 独立控制各路 SC8815 子板的启停（高电平启用）
...
4. **推荐引脚**: 优先使用 GPIO35-38、GPIO47-48 (当前未占用或低耦合的高质量IO引脚)
