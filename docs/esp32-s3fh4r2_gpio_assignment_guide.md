...
- **GPIO47**: UCM_DIN (CH442E 路由选择，低电平默认连接 MCU 通道，建议外部下拉)
- **GPIO48**: UCM_DCE (CH442E 使能控制，对应 EN#，低电平使能，建议外部下拉)
...
当前规划中，`GPIO47`/`GPIO48` 已分配给 `UCM_DIN`/`UCM_DCE`，暂不保留额外预留 IO。
...
| **USB HUB控制** | 7个 | GPIO5, GPIO17, GPIO18, GPIO39, GPIO40, GPIO47, GPIO48 |
...
| **GPIO47** | 37 | UCM_DIN | CH442E 路由选择控制 | 低电平默认连接 MCU，建议外部下拉 |
| **GPIO48** | 36 | UCM_DCE | CH442E 使能控制（EN#） | 低电平使能，建议外部下拉 |
...
3. **优先推荐**: GPIO33-37 (ESP32-S3FH4R2额外可用的通用IO引脚)
...
4. **推荐引脚**: 优先使用GPIO33-38 (ESP32-S3FH4R2可用的高质量IO引脚)
