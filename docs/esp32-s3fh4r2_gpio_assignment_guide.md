...
**功能特点**: 当前主板使用 `TPS62933DRLR` buck 生成 `FAN_VCC`，`GPIO2/FAN_EN` 高电平使能，`GPIO1/FAN_PWM` 通过反馈网络做 DC 调速，`GPIO6/FAN_TOUCH` 负责转速反馈。
...
| **GPIO1** | 6 | FAN_PWM | PWM风扇调压控制 | 25kHz PWM 输出，经 `VCTRL/R18` 注入 `TPS62933` FB |
...
3. **调速极性**: `FAN_PWM` 平均电压越高，`TPS62933` FB 越高，`FAN_VCC` 越低；固件速度百分比需映射为反相硬件 duty
...
2. **端口管理**: PSTOP_CTL1-4 (GPIO17/18/39/40) 独立控制各路 SC8815 子板的启停（高电平启用，经板上反相为 `PSTOP` 低有效）
...
4. **推荐引脚**: 优先使用GPIO33-37, GPIO47-48 (ESP32-S3FH4R2可用的高质量IO引脚)
5. **ESP32-S3FH4R2优势**: 相比其他ESP32-S3变体，多出5个常用通用GPIO (GPIO33-37)
