...
- 五向数字开关：通过 TCA6408A 接入 MCU。
  ...
  - 前面板 TCA6408A `RESET#`：前面板本地固定上拉，MCU 不控制该复位脚。
  - 引脚映射：中键 → P0；右/下/左/上 → P1/P2/P3/P4；显示 `RES` → P5；显示 `CS` → P6。
- 显示屏：160×50 像素、GC9D01 驱动的 LCD TFT 彩屏；SPI 数据/时钟与 `DC/BLK` 由 MCU 控制，`CS/RES` 优先由前面板 TCA6408A 控制；当显示控制初始化前确认前面板 TCA 不 ACK 且本次为整机冷上电时，固件可用主板原 `GPIO13/GPIO14` 作为 `CS/RES` fallback。
