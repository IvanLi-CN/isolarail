...
- 每路输出模块与主板连接：
  ...
  - `ENx`（`EN1..EN4`）→ 连接 MCU（每路独立使能信号，高电平使能对应输出模块功率级）。
  - `DM/DP` → 连接至 CH335F（USB HUB 控制器的数据线侧）。
- USB 通道切换（一路下游口）：
  - 使用 CH442E 在“外部接口”与“MCU 通道”之间切换该路 D+/D-。
  - `UCM_DIN`（MCU `GPIO33`）→ CH442E `IN`（路由选择，低电平默认连接 MCU 通道，建议外部下拉）。
  - `UCM_DCE`（MCU `GPIO34`）→ CH442E `EN#`（使能控制，低电平使能，建议外部下拉）。
  - 当前 bring-up 阶段由板级下拉保持默认路由，固件暂不主动切换该路径。

## 用户交互

- 五向数字开关：通过 TCA6408A 接入 MCU。
  - 前面板 TCA6408A I²C 地址：0x21（ADDR 接 3V3）。
  - 引脚映射：中键 → P0；右/下/左/上 → P1/P2/P3/P4。
...
- 启动默认态由板级外部下拉保持，后续若需要切到外部接口，必须由显式的策略层接管这两个 GPIO。
