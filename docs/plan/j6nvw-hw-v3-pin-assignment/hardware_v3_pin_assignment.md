...
| --- | --- | ---: | --- | --- | --- |
| I2C | I2C1_SDA | 8 | MCU →/← I2C | - | `SDA1`：输入 INA226、前面板、OUT3/OUT4 输出模块传感器 |
| I2C | I2C1_SCL | 9 | MCU →/← I2C | - | `SCL1`：输入 INA226、前面板、OUT3/OUT4 输出模块传感器 |
| I2C | I2C0_SDA | 14 | MCU →/← I2C | - | `SDA0`：主板 TCA6408A hub-sideband、OUT1/OUT2 输出模块传感器 |
| I2C | I2C0_SCL | 13 | MCU →/← I2C | - | `SCL0`：主板 TCA6408A hub-sideband、OUT1/OUT2 输出模块传感器 |
...
| Display (front TCA) | LCD_RES | - | TCA → LCD | active-low | 前面板 TCA6408A P5 |
| Display (front TCA) | LCD_CS | - | TCA → LCD | active-low | 前面板 TCA6408A P6，固件保持输出低 |
...
- 固件使用 GPIO35 释放板级 `RESET#`，并通过前面板 TCA6408A P5/P6 控制 LCD `RES/CS`。
...
### `SDA1/SCL1`（GPIO8/GPIO9）

| 器件 | 作用 | I2C 地址 | 备注 |
| --- | --- | ---: | --- |
| TCA6408A（前面板 U43） | 五向开关输入 + LCD 控制 | 0x21 | 五向：中=P0，右=P1，下=P2，左=P3，上=P4；LCD RES=P5，CS=P6（ADDR 接 3V3） |
| INA226 | 输入功率/电压/电流检测 | 0x44 | `A1=GND`、`A0=3V3` |
| PCA9545A | 4 通道 I2C 复用/隔离 | 0x70 | 下行通道隔离 4 路子板（SC8815/SW2303 地址固定） |
| OUT3/OUT4 模块传感器 | 输出模块 INA226/TMP112 | 0x42/0x4A, 0x43/0x4B | 按主板网表 OUT3/OUT4 金手指连接 |

### `SDA0/SCL0`（GPIO14/GPIO13）

| 器件 | 作用 | I2C 地址 | 备注 |
| --- | --- | ---: | --- |
| TCA6408A（主板 U43） | HUB `PWREN#/OVCUR#` 扩展 | 0x20 | P0/P2/P4/P6=`PWREN1..4#`，P1/P3/P5/P7=`OVCUR1..4#`（ADDR 接地） |
| OUT1/OUT2 模块传感器 | 输出模块 INA226/TMP112 | 0x40/0x48, 0x41/0x49 | 按主板网表 OUT1/OUT2 金手指连接 |
