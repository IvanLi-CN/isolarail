...
| USB hub | HUB_SCL | 37 | MCU ↔ HUB | digital I/O | CH335F `LED3/SCL` 复用控制信号 |
| USB hub | HUB_SDA | 36 | MCU ↔ HUB | digital I/O | CH335F `LED4/SDA` 复用控制信号 |
...
- HUB 侧带信号：Rev2.3 网表使用 `HUB_SDA/HUB_SCL = GPIO36/GPIO37`（CH335F `LED4/SDA` 与 `LED3/SCL` 复用路径）。
...
| INA226 | 输入功率/电压/电流检测 | 0x44 | `A1=3V3`、`A0=GND` |
