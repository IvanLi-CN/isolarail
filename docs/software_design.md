...
- V3 `BUZZER` 网络由 ESP32-S3 `GPIO7` 驱动；固件使用软件方波播放无源蜂鸣器音效，不占用 fan 已使用的 LEDC 外设。
...
- 前面板按键音：
  ...
  - Center 因 OCP latch、全局 `PWR_SW_TARGET=Open`、CH335F sideband gate 或端口未就绪被拒绝时播放操作拒绝音，并记录拒绝原因；
...
- 告警映射：
  ...
  - `channel_over_5a` 在任一通道 `current_ma >= 5000` 或 high-current OCP latch 时触发，按长间隔循环；
