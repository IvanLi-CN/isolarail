...
- USB 通道切换默认态（CH442E）：
  - `UCM_DIN`（`GPIO33`）默认输出低电平（默认路由到 MCU 通道）；
  - `UCM_DCE`（`GPIO34`）默认输出低电平（`EN#` 低有效，默认使能切换器）；
...
- 初始化/扫描策略（逐通道，串行）：
  ...
  2) 若 SC8815 在线：完成初始化（外部电阻 `RS1/RS2=5mΩ`，`IBUS=5A`、`IBAT=6A`，`OTG`，`450kHz`，目标 `VBUS=5V`，使能 `ADC`/`OTG`），随后 MCU 拉高该路 `ENx`（`EN1..EN4`，对应输出模块通道）以使能输出；
