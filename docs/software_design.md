...
4) 在 VIN ready 时探测前面板 `TCA6408A` 与风扇链路：
   - 当前 V3 硬件下，front panel 离线记为 `Warn/FrontPanelOffline`，禁用前面板输入任务且显示 CS/RES 不可用；
...
- 前面板 `TCA6408A` 地址为 `0x21`，P0..P4 读取五向按键，P5/P6 分别驱动显示 `RES/CS`；显示控制输出在 LCD 初始化前配置，前面板在线状态仍在输入电源 ready 后探测并记录。
...
- 门控要求：
  ...
  - 若 bus-clear 后共享总线上其它器件在线但 `0x21` 仍不 ACK，当前硬件标记 `Warn/FrontPanelOffline` 并继续 runtime，同时禁用前面板输入任务且显示 CS/RES 不可用；
...
- 前面板任务只负责读取 TCA6408A 输入并发布去抖后的按键下降沿事件，不直接持有或驱动 `EN1..EN4`；显示初始化路径单独使用同一 TCA 的 P5/P6 控制 `RES/CS`。
