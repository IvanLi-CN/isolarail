...
4) 在 VIN ready 时探测前面板 `TCA6408A` 与风扇链路：
   - 当前 V3 硬件下，front panel 离线记为 `Warn/FrontPanelOffline`，禁用前面板输入任务；若显示控制初始化前确认 `0x21` 不 ACK，且 reset reason 明确为 `ChipPowerOn`，显示链路改用原 MCU `GPIO13/GPIO14` 兜底驱动 `CS/RES`；
   - 风扇链路初始化失败记为 `Warn/FanUnavailable`，不阻断 dashboard。
5) 仅在 `VIN ready` 时初始化主板 CH335F sideband 与四路端口扫描：
   - 主板 `TCA6408A@0x20` 负责读取 CH335F `PWREN#` 与注入 `OVCUR#`；
...
- 前面板 `TCA6408A` 地址为 `0x21`，P0..P4 读取五向按键，P5/P6 分别驱动显示 `RES/CS`；固件在 LCD 初始化前通过 P5 产生低有效复位脉冲，随后将 P6 拉低并保持为屏幕 CS 使能，不在每个 SPI 事务中翻转 CS。若显示控制初始化前确认 `0x21` 不 ACK，且 reset reason 明确为 ChipPowerOn，固件改用原 MCU `GPIO13/GPIO14` 驱动显示 `CS/RES`；若任一初始化尝试进入 partial failure，则 GPIO13/GPIO14 保持高阻，避免共享网冲突；MCU-only reset 不启用该 fallback，以避免与未复位的 TCA 输出对打。
...
- 门控要求：
  ...
  - 若 bus-clear 后共享总线上其它器件在线但 `0x21` 仍不 ACK，当前硬件标记 `Warn/FrontPanelOffline` 并继续 runtime，同时禁用前面板输入任务；只有显示控制初始化前 `0x21` 不 ACK 且本次为整机冷上电时，显示才允许改用原 MCU `GPIO13/GPIO14` 兜底；
  - 若早期显示路径为 `Unavailable` 且运行期前面板探测恢复在线，固件重新配置 TCA P5/P6 并重试 LCD 初始化；若早期已启用 MCU fallback，本次启动保持 GPIO fallback，不热切换显示控制路径；
...
- 前面板任务只负责读取 TCA6408A 输入并发布去抖后的按键下降沿事件，不直接持有或驱动 `EN1..EN4`；显示初始化路径优先使用同一 TCA 的 P5/P6 控制 `RES/CS`，初始化前 TCA 不 ACK 且本次为整机冷上电时改用 MCU `GPIO13/GPIO14` fallback。
