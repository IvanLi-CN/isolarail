...
- 为避免产品在当前硬件上卡在 `PANEL PEND`，V3 固件将前面板离线记录为 `Warn/FrontPanelOffline` 并继续 runtime；后续实物验证确认显示可通过原 MCU `GPIO13/GPIO14` fallback 保持可用，但 fallback 仅在显示控制初始化前 `0x21` 不 ACK且本次为整机冷上电时启用，硬件缺陷已登记为 GitHub issue #18。
