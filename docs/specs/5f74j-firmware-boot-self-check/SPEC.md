...
- 当前 V3 硬件下，前面板缺失时必须在有限恢复失败后标记 `Warn/FrontPanelOffline`，禁用前面板输入任务，并继续 runtime；若显示控制初始化前 `0x21` 不 ACK，且 reset reason 明确为 `ChipPowerOn`，显示 CS/RES 在该状态下改用 MCU `GPIO13/GPIO14` fallback。
...
- Front panel 离线：当前 V3 硬件下标记 `Warn/FrontPanelOffline`，禁用前面板输入任务；显示控制初始化前 `0x21` 不 ACK且本次为整机冷上电时，显示 CS/RES 改用 MCU `GPIO13/GPIO14` fallback。
...
- Given 前面板 `TCA6408A` 缺失且显示控制初始化前 `0x21` 不 ACK，When 当前 V3 硬件整机冷上电启动，Then 固件标记 `Warn/FrontPanelOffline`，禁用前面板输入任务，显示改用 MCU fallback 并继续 runtime。
