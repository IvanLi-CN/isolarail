...
- sideband helper 离线时进入 degraded manual mode，owner 显式开启的端口可输出，但 `OVCUR#`
  注入不可用且 telemetry 必须保持 fault 可见。
...
- 真机：已在 `/dev/cu.usbmodem2123101` 验证 sideband fault 下可通过 owner 控制将 port 1/4
  切换到 `power_enabled=true`。
