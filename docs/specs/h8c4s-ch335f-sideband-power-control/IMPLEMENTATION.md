...
- `V1OK=high` 时进入 host-managed 模式，按 CH335F `PWREN#` 关闭或开启对应输出。
...
- 文档：`docs/software_design.md` 与本 spec 描述 standalone/host-managed 行为。
- 真机：已验证 standalone 无上游主机仍可输出。
- 真机：已验证 host `2-1.2.2` port 2/3/4 可分别驱动固件 `p1/p3/p4` 的 `pwren/en` 状态。

## Remaining Gaps

- host port 1 当前承载 MCU USB JTAG/serial 调试口，未在串口监视期间关断测试。
