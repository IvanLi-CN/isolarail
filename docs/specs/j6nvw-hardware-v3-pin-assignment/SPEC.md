...
- V3 硬件把显示屏 SPI 数据/时钟与 `DC/BLK` 接到 MCU GPIO，显示 `CS/RES` 由前面板 `TCA6408A` 控制。
...
- `LCD_RST/RES` 必须由前面板 `TCA6408A@0x21` 的 P5 输出控制，`LCD_CS` 必须由 P6 输出控制。
- MCU 侧 `GPIO13/GPIO14` 必须保持默认未分配状态，不再占用 `LCD_CS` 与 `LCD_RST/RES` 网络。
...
- 当前 V3 硬件无法在固件内硬复位前面板 `TCA6408A@0x21`；若 bus-clear 后只有 `0x21` 不 ACK，固件必须标记 `Warn/FrontPanelOffline` 并继续 runtime，同时禁用前面板输入任务且显示 CS/RES 不可用。
- 未来硬件修订引出前面板 TCA `RESET#` 或 VCCP 控制后，应撤销当前 V3 降级路径，改为硬复位恢复并要求 `0x21` 在线。

### SHOULD

- 固件日志应明确当前显示控制路径为 front-panel TCA CS/RES。
- 项目文档应标出 `SPI_BLK` 的低有效语义。

## 功能与行为规格

- 启动时固件先初始化 I2C 与前面板 `TCA6408A@0x21` 的显示控制输出，再初始化 SPI2 与 GC9D01 160x50 panel。
- `BLK` 输出低电平后背光打开。
- framebuffer 以 160x50 逻辑尺寸渲染，driver 负责 `LandscapeSwapped` 到物理坐标的转换。
- 当前 V3 硬件下，`TCA6408A@0x21` 同时影响显示 CS/RES 与前面板按键任务；离线时显示不可用，固件仍记录 `Warn/FrontPanelOffline` 并继续 runtime。

## 验收标准

- Given V3 前面板与显示屏焊接正确，When 固件启动，Then 背光点亮且 LCD 初始化日志显示 `tca cs/res, landscape-swapped`。
...
- Given 前面板 `TCA6408A@0x21` 离线，When 当前 V3 硬件启动自检，Then 日志报告 `Warn/FrontPanelOffline`，不启动按键任务并继续 runtime。
