...
- host-tools 使用本项目 source-built `isohub` CLI 与 `isohub-devd`。
...
```json
{"id":"...","ok":true,"result":{"schema":"iso-usb-hub.hardware.snapshot.v1"}}
```
...
- CLI `isohub --json diag-snapshot` 输出可被 Web 同一 schema 消费。

## 质量门槛

- `cargo +esp check`
- `cargo +esp build --release`
- `cargo test` in `tools/isohub-companion`
...
- [x] `isohub`/`isohub-devd` CLI/devd 与基础测试
