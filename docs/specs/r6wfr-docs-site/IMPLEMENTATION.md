...
主题切换通过本地 theme entry 接管，内部基础组件从 `@rspress/core/theme-original` 引用以避免 alias 递归。切换动画以真实点击点或开关中心的 CSS 像素坐标为原点，不受设备像素比影响；结束后主动清理 root animation，防止后续轮次复用残留的 `forwards` 填充状态。首页亮色与暗色产品图由 CSS 按当前主题互斥显示，并通过 `DocsThemeFigure` 的 Vite asset import 生成，因而会自动跟随 `DOCS_BASE` 部署到子路径。
