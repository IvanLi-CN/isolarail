...
- 四路 IP6557 子板的专用驱动/遥测实现（另立 bring-up 任务承接）。
...
- 当前任务的最新调整：`vin_on` 的实时状态刷新与 10 s 遥测解耦；等待 `VIN_ON` 的旧调用点必须显式等待 `true` 或自行超时，不能把初始 `false` 当成已就绪事件。
...
- 已落地的 V3 适配：
  ...
  - `power_in` 的 `vin_on` 状态以约 `500 ms` 周期刷新；10 s 状态日志仅承担遥测汇报，不再决定 UI/模块路径的即时电源状态。
...
- 尚未完成：
  - 台架上电验证；
  - 串口日志对照 `front_panel`/模块侧 `bringup-pending` 状态的现场证据；
  - IP6557 子板专用 bring-up 任务与验收规格。
...
- 风险：IP6557 子板的寄存器初始化、测量地址与告警联动尚未在当前任务内冻结。
- 需要决策的问题：
  - IP6557 子板的专用 bring-up 规格由哪个后续任务承接？
