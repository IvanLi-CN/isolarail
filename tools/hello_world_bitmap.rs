//! HELLO WORLD Bitmap Generator
//! 
//! Generates a 120×30 pixel bitmap with "HELLO WORLD" text using gradient colors
...
fn get_gradient_color(x: usize, y: usize) -> u16 {
    // Create a blue to cyan gradient horizontally
...
fn main() -> std::io::Result<()> {
    ...
    // Background color (bright blue - much more visible)
    let bg_color = rgb888_to_rgb565(0, 128, 255);
    
    // Fill background
    for y in 0..HEIGHT {
        for x in 0..WIDTH {
            let pixel_idx = (y * WIDTH + x) * BYTES_PER_PIXEL;
            let color_bytes = bg_color.to_le_bytes();
                ...
                for col in 0..5 {
                    if (font_row >> (4 - col)) & 1 != 0 {
                        ...
                        if pixel_x < WIDTH && pixel_y < HEIGHT {
                            ...
                            let color = get_gradient_color(pixel_x, pixel_y);
    ...
    let mut file = File::create("hello_world_120x40.bin")?;
    ...
    println!("Generated hello_world_120x40.bin");
